...
if os.environ.get('BANANA_MODEL_KEY'):
    BANANA_MODEL_KEY = os.environ.get('BANANA_MODEL_KEY')

if os.environ.get('BANANA_API_KEY'):
...
def call_carrot(image_base64):
    """Call carrot hosted on banana to get image description"""

    model_parameters = {
                        "imageByteString":image_base64, #image for the model
    ...
    out = banana.run(BANANA_API_KEY, BANANA_MODEL_KEY, model_parameters)
    print(out)
    capt = out["modelOutputs"][0]["caption"]
    return capt


@auth(USER_ID)
async def echo_image(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Echo the user message containing image."""

    image = await update.message.photo[0].get_file()
    buffered = BytesIO()
    
    await image.download_to_memory(buffered)
    image_base64 = base64.b64encode(buffered.getvalue()).decode('utf-8')
    caption = call_carrot(image_base64)
    caption = f'Assume you see an image with the following- {caption}. what is the image about (be concise and confident)?'
    # Send the message to OpenAI
